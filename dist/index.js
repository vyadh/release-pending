...
// src/release.ts
async function createDraftRelease(context, tagName, targetCommitish, name) {
  const response = await context.octokit.rest.repos.createRelease({
    owner: context.owner,
    repo: context.repo,
    tag_name: tagName,
    // todo odd that this is required according to the docs, need to verify
...
// src/caching-async-iterable.ts
var CachingAsyncIterable = class {
  constructor(source) {
    this.cache = [];
...
// src/releases.ts
var DEFAULT_PER_PAGE2 = 30;
var MAX_PAGES = 5;
var Releases = class {
  constructor(source, maxReleases) {
    this.source = source;
   ...
   */
  async findLastDraft(targetCommitish) {
    for await (const release of this.source) {
  ...
  async findLast(targetCommitish) {
    return this.find(
...
function fetchReleases(context, perPage) {
  const maxReleases = (perPage ?? DEFAULT_PER_PAGE2) * MAX_PAGES;
  return new Releases(new CachingAsyncIterable(createReleasesGenerator(context, perPage)), maxReleases);
}
async function* createReleasesGenerator(context, perPage) {
  const iterator2 = context.octokit.paginate.iterator(context.octokit.rest.repos.listReleases, {
    owner: context.owner,
    repo: context.repo,
    per_page: perPage ?? DEFAULT_PER_PAGE2
  });
  for await (const response of iterator2) {
    for (const release of response.data) {
      yield mapRelease2(release);
    }
  }
}
function mapRelease2(releaseData) {
  return {
    id: releaseData.id,
    tagName: releaseData.draft ? null : releaseData.tag_name,
    targetCommitish: releaseData.target_commitish,
...
// src/version-bump-inference.ts
function inferImpactFromPRs(prs) {
  return maxImpact(prs.map(inferVersionImpactFromPR));
}
function inferVersionImpactFromPR(pr) {
...
async function simulate(octokit, args) {
  if (args.length < 3) {
  ...
  const context = { octokit, owner, repo, branch };
...
async function showReleases(octokit, args) {
  if (args.length < 2) {
  ...
  try {
    const context = { octokit, owner, repo, branch: "placeholder" };
    const releases = fetchReleases(context);
...
async function showPullRequests(octokit, args) {
  if (args.length < 4) {
