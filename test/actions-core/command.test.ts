...
describe("@actions/core/src/command", () => {
  ...
  it("command escapes message", () => {
    ...
    command.issueCommand("some-command", {}, "percent % percent % cr \r cr \r lf \n lf \n")
    assertWriteCalls([`::some-command::percent %25 percent %25 cr %0D cr %0D lf %0A lf %0A${os.EOL}`])

    // Verify literal escape sequences
    process.stdout.write = vi.fn()
    command.issueCommand("some-command", {}, "%25 %25 %0D %0D %0A %0A")
    assertWriteCalls([`::some-command::%2525 %2525 %250D %250D %250A %250A${os.EOL}`])
  })
  ...
  it("command escapes property", () => {
    ...
    // Verify literal escape sequences
    process.stdout.write = vi.fn()
    command.issueCommand("some-command", {}, "%25 %25 %0D %0D %0A %0A %3A %3A %2C %2C")
    assertWriteCalls([`::some-command::%2525 %2525 %250D %250D %250A %250A %253A %253A %252C %252C${os.EOL}`])
  })
  ...
  it("command with message and properties", () => {
    command.issueCommand("some-command", { prop1: "value 1", prop2: "value 2" }, "some message")
    assertWriteCalls([`::some-command prop1=value 1,prop2=value 2::some message${os.EOL}`])
  })
  ...
  it("command with two properties", () => {
    command.issueCommand("some-command", { prop1: "value 1", prop2: "value 2" }, "")
    assertWriteCalls([`::some-command prop1=value 1,prop2=value 2::${os.EOL}`])
  })

  it("command with three properties", () => {
    command.issueCommand("some-command", { prop1: "value 1", prop2: "value 2", prop3: "value 3" }, "")
    assertWriteCalls([`::some-command prop1=value 1,prop2=value 2,prop3=value 3::${os.EOL}`])
  })
