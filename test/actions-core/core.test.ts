...
describe("@actions/core", () => {
  ...
  it("getInput trims whitespace when option is explicitly true", () => {
    expect(core.getInput("with trailing whitespace", { trimWhitespace: true })).toBe("some val")
  })

  it("getInput does not trim whitespace when option is false", () => {
    expect(core.getInput("with trailing whitespace", { trimWhitespace: false })).toBe("  some val  ")
  })
  ...
  it("setOutput produces the correct command and sets the output", () => {
    ...
    verifyFileCommand(command, `my out<<${DELIMITER}${os.EOL}out val${os.EOL}${DELIMITER}${os.EOL}`)
  })
  ...
  it("setOutput handles boolean inputs", () => {
    ...
    verifyFileCommand(command, `my out<<${DELIMITER}${os.EOL}true${os.EOL}${DELIMITER}${os.EOL}`)
  })
  ...
  it("setOutput handles number inputs", () => {
    ...
    verifyFileCommand(command, `my out<<${DELIMITER}${os.EOL}5${os.EOL}${DELIMITER}${os.EOL}`)
  })
